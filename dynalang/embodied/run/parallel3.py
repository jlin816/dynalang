...
def parallel3(agent, make_logger, make_replay, make_env, num_envs, args):
  workers = []
  barrier = threading.Barrier(2)
  for i in range(num_envs):
    worker = embodied.distr.Process(parallel_env, i, make_env, args, True)
    worker.start()
...
def parallel_actor(agent, barrier, args):
  ...
  logger = embodied.distr.Client(
      'ipc:///tmp/logger', name='ActorLogger', connect=True,
      maxinflight=8 * args.actor_threads)
  replay = embodied.distr.Client(
      'ipc:///tmp/replay', name='ActorReplay', connect=True,
      # maxinflight=2 * args.actor_threads)
      maxinflight=8 * args.actor_threads)
  ...
  def donefn(trans):
    ...
    logger.trans(trans)  # TODO: This one is slow!
    if should_log():
      stats = {}
...
def parallel_learner(agent, barrier, args):
  ...
  logger = embodied.distr.Client(
      'ipc:///tmp/logger', name='LearnerLogger', connect=True)
  barrier.wait()
  ...
  def parallel_dataset(prefetch=10):  # TODO: prefetch number?
    replay = embodied.distr.Client(
        'ipc:///tmp/replay', name=f'LearnerReplay{len(replays)}', connect=True)
  ...
  while True:
    ...
    agg.add(mets)  # TODO: Aggregate train metrics in the logger server?
...
def parallel_replay(make_replay, args):
  ...
  should_log = embodied.when.Clock(args.log_every)
  logger = embodied.distr.Client(
      'ipc:///tmp/logger', name='ReplayLogger', connect=True,
      maxinflight=1)
...
def parallel_logger(make_logger, args):
  ...
  def transfn(trans):
    ...
    for i, addr in enumerate(envids):
      ...
      # TODO: This section can be really slow
      with embodied.timer.section('logs4'):
        for key in trans.keys():
          if not args.log_zeros:
            if key in nonzeros:
              pass
            elif np.any(tran[key] != 0):
              nonzeros.add(key)
            else:
              continue
  ...
  server = embodied.distr.Server('ipc:///tmp/logger', name='Logger')
...
def parallel_env(envid, make_env, args, logging=False):
  ...
  if logging:
    logger = embodied.distr.Client(
        'ipc:///tmp/logger', name=f'{name}Logger', connect=True,
        maxinflight=1)
  fps = embodied.FPS()
  if envid == 0:
    usage = embodied.Usage(**args.usage.update(nvsmi=False))
  ...
  actor = embodied.distr.Client(
      addr, envid, name, args.ipv6, pings=10, maxage=60, connect=True)
